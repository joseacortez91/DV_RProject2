...
earth.df <- filter(df, RADIUS != 'null' & MASS != 'null') %>% transform(ECCENTRICITY = as.numeric(as.character(ECCENTRICITY))) %>% transform(MASS = as.numeric(as.character(MASS))) %>% transform(RADIUS = as.numeric(as.character(RADIUS))) %>% transform(ORB_PERIOD = as.numeric(as.character(ORB_PERIOD)))
